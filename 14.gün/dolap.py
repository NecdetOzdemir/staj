...
# ==================== 4.1. KAPI AÇIK KALMA SÜRE HESAPLAMA (YENİ) ====================
def kapi_acik_kalma_suresini_hesapla(baslangic_index, df_sorted):
    ...
    baslangic_sicaklik = df_sorted['Value'].iloc[baslangic_index]
    
    # En yüksek sıcaklığı bul (tepe noktası)
    max_sicaklik = baslangic_sicaklik
    max_sicaklik_index = baslangic_index
    
    for i in range(baslangic_index, min(baslangic_index + 30, len(df_sorted))):  # Sonraki 30 noktayı kontrol et
        if df_sorted['Value'].iloc[i] > max_sicaklik:
            max_sicaklik = df_sorted['Value'].iloc[i]
            max_sicaklik_index = i
    ...
    for i in range(max_sicaklik_index + 1, len(df_sorted)):
        # En az 0.5°C düşüş kontrolü
        if df_sorted['Value'].iloc[i] <= max_sicaklik - KAPI_ACIK_DUSUS_ESIGI:
            bitis_zamani = df_sorted['Tarih'].iloc[i]
            sure_dakika = (bitis_zamani - df_sorted['Tarih'].iloc[baslangic_index]).total_seconds() / 60
        ...
        if i > max_sicaklik_index + 5:  # Tepe noktasından sonra en az 5 nokta geçti
            ...
            recent_values = df_sorted['Value'].iloc[i-5:i]
        ...
        if trend == "yukselen":
            if fark > ESIK_YUKSELEN:
                ...
                if fark >= ESIK_ELEKTRIK_KESINTISI:
                    ...
                    sure_dakika, bitis_zamani = kapi_acik_kalma_suresini_hesapla(i, df_sorted)
...
def grupla_anomaliler(anomaliler, zaman_araligi_dakika=GRUP_ZAMAN_ARALIGI, df_sorted=None):
    ...
    for olay in kapi_acik_kaldi_olaylari:
        grup_ozetleri.append({
            'baslangic': olay['tarih'],
            'bitis': olay['bitis_zamani'],
            'sure_dakika': olay['sure_dakika'],
